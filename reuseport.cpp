...
// Set reuse_addr flag for socket as workaround for inability to add sockets
// into reuse_port group after we assign BPF for it
bool set_reuse_addr_flag = false;

// In this case we will follow documented behaviour which sadly does not work
// without reuse_addr
...
bool load_bpf(int sockfd, uint32_t threads_per_port) {
    ...
    if (attach_filter_result != 0) {
        std::cout << "Can't attach reuse port BPF filter "
...
bool create_and_bind_socket(std::size_t thread_id, const std::string& host, unsigned int port, uint32_t threads_per_port, int& sockfd) {
    ...
    std::cerr << "Successful bind" << std::endl;
...
void capture_traffic_from_socket(int sockfd, std::size_t thread_id) {
    ...
    while (true) {
        ...
        // logger << log4cpp::Priority::ERROR << "Received " << received_bytes << "
        // with UDP server";
